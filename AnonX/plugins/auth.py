...
async def auth(client, message: Message, _):
    if not message.reply_to_message:
        ...
        if token not in _check:
            ...
            await message.reply_sticker("CAACAgEAAx0CbWlwNAACAbFkbHU9rIQYls4YxGlAOF7pWiPZigACgAADwKwIIwbvvmK1zSleLwQ")
    ...
    if token not in _check:
        ...
        if get:
            if user_id not in get:
                get.append(user_id)
        await save_authuser(message.chat.id, token, assis)
        await message.reply_sticker("CAACAgEAAx0CbWlwNAACAdtkbHhjSCnAnxfTpR4pzWuYRK4qaAACYQADwKwII0_MBvGSRUXNLwQ")
        return await message.reply_text("**» ᴀᴅᴅᴇᴅ ᴛᴏ ᴀᴜᴛʜᴏʀɪsᴇᴅ ᴜsᴇʀs ʟɪsᴛ ᴏꜰ ʏᴏᴜʀ ɢʀᴏᴜᴘ.**")
    else:
        await message.reply_text("**» ᴀʟʀᴇᴀᴅʏ ɪɴ ᴛʜᴇ ᴀᴜᴛʜᴏʀɪsᴇᴅ ᴜsᴇʀs ʟɪsᴛ.**")
...
async def unauthusers(client, message: Message, _):
    if not message.reply_to_message:
        ...
        if get:
            if user.id in get:
                get.remove(user.id)
        if deleted:
            await message.reply_sticker("CAACAgEAAx0CbWlwNAACAcpkbHg3Q118ugAB79O20edUzFwn71kAAqIBAAJmDvBEcXXLw9CycUwvBA")
    ...
    if get:
        if user_id in get:
            get.remove(user_id)
    if deleted:
        await message.reply_sticker("CAACAgEAAx0CbWlwNAACActkbHg7rRq6MMY6LQABbopaFAGBFFUAAlkAA8CsCCN_MpkmR4j1ES8E")
        return await message.reply_text("**» ʀᴇᴍᴏᴠᴇᴅ ꜰʀᴏᴍ ᴀᴜᴛʜᴏʀɪsᴇᴅ ᴜsᴇʀs ʟɪsᴛ ᴏꜰ ᴛʜɪs ɢʀᴏᴜᴘ.**")
    else:
        return await message.reply_text("**» ᴛᴀʀɢᴇᴛᴇᴅ ᴜsᴇʀ ɪs ɴᴏᴛ ᴀɴ ᴀᴜᴛʜᴏʀɪsᴇᴅ ᴜsᴇʀ.**")
